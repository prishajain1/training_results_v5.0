...
if [ ${NVTX_FLAG} -gt 0 ]; then
  NSYS_BINARY=$(which nsys)
  if [[ -e "/nsight/bin/nsys" ]]; then
      NSYS_BINARY="/nsight/bin/nsys"
  fi
  NSYSCMD="${NSYS_BINARY} profile --capture-range cudaProfilerApi --capture-range-end stop --sample=none --cpuctxsw=none  --trace=cuda,nvtx  --force-overwrite true --output /results/language_model_pytorch_${DGXNNODES}x${DGXNGPU}x${BATCHSIZE}_${DATESTAMP}_${SLURM_PROCID}_${SYNTH_DATA}_${DISABLE_CG}.nsys-rep "
fi
...
if [[ ${SYNTH_DATA} -ge 1 ]]; then
 ...
 EXTRA_PARAMS+=" --init_checkpoint=/workspace/phase1/model.ckpt-28252.pt "
fi
