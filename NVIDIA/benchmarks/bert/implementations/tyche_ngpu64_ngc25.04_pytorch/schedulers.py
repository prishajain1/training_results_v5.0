...
class LinearWarmUpScheduler(LRScheduler):
    ...
    def __init__(self, optimizer, warmup, total_steps, last_epoch=-1):
        ...
        mllogger.log_event(key=mllogger.constants.OPT_LR_WARMUP_STEPS, value=total_steps*warmup, sync=False)
...
class LinearWarmupPolyDecayScheduler(LRScheduler):
    ...
    def __init__(self, optimizer, start_warmup_steps, warmup_steps, total_steps, end_learning_rate=0.0, degree=1.0, last_epoch=-1):
        ...
        mllogger.event(key=mllogger.constants.OPT_LR_WARMUP_STEPS, value=self.num_warmup_updates, sync=False)
        mllogger.event(key=mllogger.constants.OPT_LAMB_LR_DECAY_POLY_POWER, value=degree, sync=False)
        mllogger.event(key=mllogger.constants.START_WARMUP_STEP, value=self.start_warmup_steps, sync=False)
