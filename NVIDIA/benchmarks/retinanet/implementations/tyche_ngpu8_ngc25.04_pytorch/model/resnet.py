...
from .nvfuser_fn import bn_relu_jit, bn_add_relu_jit, bn_bn_add_relu_jit, bn_relu_wrapper
from apex.contrib.conv_bias_relu import ConvFrozenScaleBiasReLU
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.cudnn_runtime_fusion = kwargs['cudnn_runtime_fusion']

    def forward(self, x: Tensor) -> Tensor:
        identity = x

        if self.cudnn_runtime_fusion:
            out = ConvFrozenScaleBiasReLU(x, self.conv1.weight, self.bn1.scale, self.bn1.bias_term,
                                          self.conv1.padding[0], self.conv1.stride[0])
        else:
            out = self.conv1(x)
            out = self.bn1(out)
            out = self.relu(out)
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.jit = kwargs['jit']
        self.cudnn_runtime_fusion = kwargs['backbone_fusion']
        ...
        self.layer1 = self._make_layer(block, 64, layers[0],
                                       cudnn_runtime_fusion=self.cudnn_runtime_fusion)
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False, cudnn_runtime_fusion: bool = False) -> nn.Sequential:
        ...
        layers = [block(self.inplanes, planes, stride, downsample, self.groups,
                        self.base_width, previous_dilation, norm_layer, cudnn_runtime_fusion=cudnn_runtime_fusion)]
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, cudnn_runtime_fusion=cudnn_runtime_fusion))

        return nn.Sequential(*layers)

    def _forward_impl(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()]
        if self.fused_conv_scale_bias_relu:
            x = ConvFrozenScaleBiasReLU(x, self.conv1.weight, self.bn1.scale, self.bn1.bias_term, self.conv1.padding[0], self.conv1.stride[0])
        else:
            x = self.conv1(x)
            x = self.bn1(x)
