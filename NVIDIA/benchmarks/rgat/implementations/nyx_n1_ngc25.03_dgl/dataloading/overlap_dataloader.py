        ...
        while not done_event.is_set():
            ...
            if batch == None:
                ...
                with torch.cuda.stream(stream):
                    ...
                    exception = None
                    embed_done_launch_event.set()
                    if pre_get_dynamic_offsets_indices:
                        dynamic_offsets_indices = get_offsets_indices(actual_batch)
                    else:
                        dynamic_offsets_indices = None

            _try_put_data(
                embed_queue,
                (actual_batch, dynamic_offsets_indices, batch_inputs, batch_labels, exception),
                done_event,
            )
...
class PrefetchInterleaver:
    def __init__(
        self, dataloader, sample_stream, embed_stream, feature_extractor, device, features, 
        repeat_input_after=-1, pre_get_dynamic_offsets_indices=False
    ):
        ...
        self.pre_get_dynamic_offsets_indices = pre_get_dynamic_offsets_indices
    def __iter__(self):
        self._status_init()
        ...
        embed_thread = threading.Thread(
            ...
            args=(
                ...
                self.pre_get_dynamic_offsets_indices
            ),
            daemon=True,
        )
        sample_thread.start()
    ...
    def _status_init(self):
        ...
        self.dynamic_offsets_indices = None
    ...
    def get_dynamic_offsets_indices(self):
        return self.dynamic_offsets_indices
