...
model.add(
    hugectr.Input(
        ...
        data_reader_sparse_param_array=[
            hugectr.DataReaderSparseParam("data{}".format(i), MULTI_HOT_SIZES[i], True, 1) for i in range(NUM_TABLE)
        ],
    )
)
...
for i in range(NUM_TABLE):
    embedding_table_list.append(
        hugectr.EmbeddingTableConfig(name=str(i), max_vocabulary_size=TABLE_SIZE_ARRAY[i], ev_size=args.ev_size)
    )
...
mllogger.event(
    ...
    value=mlperf_logger.get_row_count(args.train_data, num_columns, 4),
    metadata={mllog_constants.EPOCH_NUM: 0.0},
)
mllogger.event(
    key=mllog_constants.EVAL_SAMPLES,
    value=mlperf_logger.get_row_count(args.val_data, num_columns, 4),
    metadata={mllog_constants.EPOCH_NUM: 0.0},
)
model.fit(
