...
VALIDATION_BATCHSIZE=${VALIDATION_BATCHSIZE:-"4096"} # local batch size for evaluation, should already be tuned

BATCHSIZE=${BATCHSIZE:-"512"} # local batch size
...
CUDA_GRAPH_ESTIMATION_BATCHES=${CUDA_GRAPH_ESTIMATION_BATCHES:-"20"}
CUDA_GRAPH_PADDING_SIGMA=${CUDA_GRAPH_PADDING_SIGMA:-"3"}
EVAL_CUDA_GRAPH=${EVAL_CUDA_GRAPH:-"1"}
...
WARMUP_MODEL=${WARMUP_MODEL:-"0"}  
GC_THRESHOLD_MULTIPLIER=${GC_THRESHOLD_MULTIPLIER:-"1"} # --gc_threshold_multiplier, int
...
if [ "${USE_CUDA_GRAPH}" -gt 0 ]; then
    CMD_SUFFIX=(
        ${CMD_SUFFIX[@]} 
        --use_cuda_graph 
        --cuda_graph_estimation_batches $CUDA_GRAPH_ESTIMATION_BATCHES
        --cuda_graph_padding_sigma $CUDA_GRAPH_PADDING_SIGMA    
    )
fi

if [ "${WARMUP_MODEL}" -gt 0 ]; then
    CMD_SUFFIX=(${CMD_SUFFIX[@]} --warmup_model)
fi
...
if [[ ${NVTX_FLAG:-0} -gt 0 || ${NSYS_FLAG:-0} -gt 0 ]]; then
  NSYS_OUT="/results/graph_neural_network_pytorch_${DGXNNODES}x${DGXNGPU}x${BATCHSIZE}_${SLURM_JOBID}_${SLURM_PROCID}_${DATESTAMP}.nsys-rep"
  PRE_CMD=(
    'nsys' 'profile' 
...
COMMAND=(
    ...
    --validation_batch_size ${VALIDATION_BATCHSIZE} --batch_size ${BATCHSIZE} --seed ${SEED}
    --gatconv_backend ${GATCONV_BACKEND} --cugraph_switches ${CUGRAPH_SWITCHES} --pad_node_count_to ${PAD_NODE_COUNT_TO}
    --gc_threshold_multiplier ${GC_THRESHOLD_MULTIPLIER}
...
${LOGGER:-} ${BINDCMD:-} ${COMMAND[@]}; ret_code=$?
