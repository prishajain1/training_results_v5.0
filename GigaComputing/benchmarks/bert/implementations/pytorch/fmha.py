...
class FMHAFun(torch.autograd.Function):
    @staticmethod
    def forward(ctx, qkv, cu_seqlens, p_dropout, max_s, is_training, set_zero=False):
    ...
    def backward(ctx, dout):
        ...
        if ctx.is_nl:
            dqkv, dp, dkv = mha.bwd_nl(dout, qkv, S_dmask, ctx.cu_seqlens, ctx.p_dropout, ctx.max_s, ctx.set_zero)
        else:
            dqkv, dp = mha.bwd(dout, qkv, S_dmask, ctx.cu_seqlens, ctx.p_dropout, ctx.max_s, ctx.set_zero)
...
class FMHA(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.set_zero = config.packed_samples
    ...
    def forward(self, hidden_states, cu_seqlens, max_s, is_training=True):
        ...
        if not self.fuse_bias:
            ...
            if 'fused_dense_function' in dir(fused_dense):
                ...
                qkv = fused_dense.fused_dense._fused_dense(hidden_states, Wqkv, Bqkv)
