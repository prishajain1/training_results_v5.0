...
from mlperf_logging.mllog.constants import (EPOCH_START, EPOCH_STOP, EVAL_START, EVAL_STOP, EVAL_ACCURACY, SUCCESS, ABORTED, RUN_STOP)
...
def preprocessing(images, targets, model_ptr, data_layout):
    # Used for synthetic dataset and graph capture, performance can be improved
    target_per_image = []
...
def train_one_epoch(model, optimizer, scaler, data_loader, device, epoch, train_group, train_group_gloo, args,
                    graphed_model=None, static_input=None, static_loss=None, static_prologues_out=None,
                    sbridge=SBridge(), status=ABORTED):
    ...
    if args.cuda_profiler and args.cuda_profiler_start <= 0:
        torch.cuda.profiler.start()
    model.train()
    metric_logger = utils.MetricLogger(max_iters=args.max_iters_per_epoch, delimiter="  ")
    if args.metric_loss:
    ...
    lr_scheduler = None
    if not args.cuda_graphs and epoch < args.warmup_epochs:
    ...
    for images, targets in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        if args.async_coco and not (metric_logger.current_iter % args.async_coco_check_freq):
            ...
            if args.target_map and accuracy and accuracy >= args.target_map and status != SUCCESS:
                ...
                # Power benchmark needs to continue the epoch after run_stop even when convergence is reached
                if not args.power_benchmark:
                    break

        sbridge.stop_prof(SBridge.ITER_TIME)

        if args.cuda_profiler and args.cuda_profiler_start == (metric_logger.current_iter + 1):
            torch.cuda.profiler.start()
        if args.cuda_profiler and args.cuda_profiler_stop == metric_logger.current_iter:
            torch.cuda.profiler.stop()

    sbridge.stop_epoch_prof()
    if args.cuda_profiler and args.cuda_profiler_stop <= -1:
        torch.cuda.profiler.stop()
    ...
    if summary['samples'] > 0:
        throughput = args.num_train_ranks * summary['samples'] / (summary['end_time'] - summary['start_time'])
...
def evaluate(model, data_loader, device, epoch, eval_group, args,
    ...
    if args.cuda_profiler_eval and args.cuda_profiler_start <= 0:
        torch.cuda.profiler.start()
    mllogger.start(key=EVAL_START, value=epoch, metadata={"epoch_num": epoch},
                   sync=True, sync_group=eval_group)

    n_threads = torch.get_num_threads()
    # TODO: Make paste_masks_in_image run on the GPU
    torch.set_num_threads(1)
    cpu_device = torch.device("cpu")
    model.eval()
    metric_logger = utils.MetricLogger(max_iters=args.max_eval_iters_per_epoch, delimiter="  ")
    header = 'Test:'
    ...
    for images, targets in metric_logger.log_every(data_loader, args.eval_print_freq, header):
        ...
        if args.cuda_profiler_eval and args.cuda_profiler_start == (metric_logger.current_iter + 1):
            torch.cuda.profiler.start()
        if args.cuda_profiler_eval and args.cuda_profiler_stop == metric_logger.current_iter:
            torch.cuda.profiler.stop()
    if args.cuda_profiler_eval and args.cuda_profiler_stop <= -1:
        torch.cuda.profiler.stop()
