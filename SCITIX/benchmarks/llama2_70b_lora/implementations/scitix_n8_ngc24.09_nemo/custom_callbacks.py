...
class MetricsLogger(Logger):
    ...
    def log_hyperparams(self, params, *args, **kwargs):
        ...
        self.mllogger.event(
            ...
            value=np.load("/data/dataset/dataset_llama2-70b-lora/gov_report/train.npy", allow_pickle=True).shape[0],
        )
        self.mllogger.event(
            key=constants.EVAL_SAMPLES,
            #value=np.load("/data/validation.npy", allow_pickle=True).shape[0],
            value=np.load("/data/dataset/dataset_llama2-70b-lora/gov_report/validation.npy", allow_pickle=True).shape[0],
        )
        self.mllogger.event(
