...
if [ ! -n "${SLURM_LOCALID-}" ]; then
    ...
    if [ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]; then
        ...
        if [[ "${ENABLE_CPU_BINDING:-1}" == "1" ]]; then
            CPU_BIND='--cpu=bind_cpu_topology.sh --mem=bind_mem_topology.sh'
        fi

        COMMAND=( "/bin/bash" "/workspace/gnn/bindpcie" ${CPU_BIND} ${IB_BIND} -- ${PRE_CMD[@]} 'python3' '-u' ${COMMAND[@]})
    else
...
echo "RESULT,$result_name,,$result,HPE,$start_fmt"
