...
if [[ -n "${SLURM_LOCALID-}" ]] && [[ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]]; then
    ...
    #CMD=( 'bindpcie' '--cpu=node' ${IB_BIND} '--' ${NSYSCMD} 'python' '-u' ) #HPE change
    CMD=( './bindpcie' '--cpu=bind_cpu_topology.sh' '--mem=bind_mem_topology.sh' ${IB_BIND} '--' ${NSYSCMD} 'python' '-u' ) #HPE change
else
...
if [ "${SYNTH_DATA}" -eq 0 ] && [ "${SKIP_EVAL}" -eq 0 ]; then
...
if [ "$node_rank" -eq 0 ] && [ "$local_rank" -eq 0 ]; then
    ...
    vendor="HPE"
    echo "RESULT,$result_name,$runtime,$vendor,$start_fmt"
fi
